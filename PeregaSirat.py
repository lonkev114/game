...
win_image = pygame.image.load("Kartinki_muzyka/perage.jpg")
win_image = pygame.transform.scale(win_image, (w, h))
duck_image = pygame.image.load("Kartinki_muzyka/duck.png")
duck_image = pygame.transform.scale(duck_image, (duck_w, duck_h))
background_image = pygame.image.load("Kartinki_muzyka/background.jpg")
background_image = pygame.transform.scale(background_image, (w, h))
game_screen_bg = pygame.image.load("Kartinki_muzyka/perega.jpg")
game_screen_bg = pygame.transform.scale(game_screen_bg, (w, h))
# Загрузка звуков
impact = pygame.mixer.Sound("Kartinki_muzyka/Meat_Hook_impact.mp3.mpeg")
pobeda = pygame.mixer.Sound("Kartinki_muzyka/aplodismentyi-nebolshoy-gruppyi-lyudey-s-radostnyimi-krikami.mp3")
shoot_sound = pygame.mixer.Sound("Kartinki_muzyka/Meat_Hook.mp3")
music_files = ["Kartinki_muzyka/Mikhail_SHufutinskijj_-_YA_kalendar_ya_kalendar_ya_kalendar_ya_kalendar_70894577.mp3", "Kartinki_muzyka/Лепс, Алегрова vs Bring Me The Horizon -- Я тебе не Sleepwalking (online-audio-converter.com).mp3", "Kartinki_muzyka/g3ox-em-gigachad-theme-phonk-house-version-mp3.mp3"]
...
def main(level, player_name):
    ...
    while running:
        ...
        if (keys[pygame.K_LEFT] or keys[pygame.K_a]) and crosshair_x > 0:
            crosshair_x -= crosshair_speed
        if (keys[pygame.K_RIGHT] or keys[pygame.K_d]) and crosshair_x < w:
            crosshair_x += crosshair_speed
        if (keys[pygame.K_UP] or keys[pygame.K_w]) and crosshair_y > 0:
            crosshair_y -= crosshair_speed
        if (keys[pygame.K_DOWN] or keys[pygame.K_s]) and crosshair_y < h:
            crosshair_y += crosshair_speed
